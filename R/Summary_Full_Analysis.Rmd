...
ggsave(filename = "Ill_Prev_Comb2.png", path = "Dose Response",width = 40,height = 10,)
print(Ill_Prev_Comb)
```
...
ggsave(filename = "Food_sec2.png", path = "Dose Response",width = 8,height = 6,)
print(Food_sec)
...
ST_Comb_Analysis_Log<-bind_rows(ST_ON_Analysis_Log,ST_OFF_Analysis_Log,ST_ONWash_Analysis_Log,ST_ONWr_Analysis_Log,ST_OFFWash_Analysis_Log,ST_OFFWr_Analysis_Log,ST_Exc_Analysis_Log)
...
ggsave(filename = "Wrapped_Histograms2.png", path = "Dose Response",width = 10,height = 10,)
...
tips4<-bind_rows(tipsON,tipsOFF,tipsONWash,tipsONWr,tipsOFFWash, tipsOFFWr,tipsExc)
...
ggsave(filename = "barchart_Expos_2.png", path = "Dose Response",width = 10,height = 6,)
print(barchart_Expos)

colors_11<-c("dodgerblue4","dodgerblue3","lightskyblue1","palegreen4","palegreen3","darkseagreen1", "Orange")
...
ggsave(filename = "Barchart_Expos2_2.png", path = "Dose Response",width = 8,height = 6,)
...
List_Summary_Table<-list(ST_ON_Analysis,ST_OFF_Analysis,ST_ONWash_Analysis,ST_ONWr_Analysis,ST_OFFWash_Analysis,ST_OFFWr_Analysis,ST_Exc_Analysis)

  Treatment_Names<-c("ON", "OFF", "ONWash","ONWr", "OFFWash","OFFWr", "XExc")
...
List_Percentages_Dist<-list(tipsON,tipsOFF,tipsONWash,tipsONWr,tipsOFFWash, tipsOFFWr,tipsExc)
...
```{r}
binded_prevalences<-bind_rows(df_ill_Week_ON_Box_Prev,df_ill_Week_OFF_Box_Prev,df_ill_Week_ONWash_Box_Prev,df_ill_Week_ONWr_Box_Prev,df_ill_Week_OFFWash_Box_Prev,df_ill_Week_OFFWr_Box_Prev,df_ill_Week_Exc_Box_Prev)

binded_prevalences$Prev<-binded_prevalences$Pre*100


colors_12<-c('STOFF'="dodgerblue4",'OFFWash' ="dodgerblue3",'OFFWr'="dodgerblue1",'STON' ="darkseagreen","ONwash" ="palegreen3","ONWr"="palegreen2","Exc" = "Orange" )
...
Dose_Hist_Stacked<-ggplot(data = binded_prevalences, mapping = aes(x= Prev, fill=factor(Int,c('STOFF','OFFWash', 'OFFWr',  'STON', 'ONwash',  'ONWr', 'Exc') )))+
...
ggsave(filename = "Dose_Hist_Stacked.png", path = "Dose Response",width = 10,height = 6,)
print(Dose_Hist_Stacked)


Facet_DR_Hist<-ggplot(data = binded_prevalences, mapping = aes(x= Prev))+
  geom_histogram(bins=15,boundary = .001, color= "black")+
  facet_wrap(~factor(Int,c('STOFF','OFFWash', 'OFFWr',  'STON', 'ONwash',  'ONWr', 'Exc')),  labeller = as_labeller(Histolabels2))+
...
ggsave(filename = "Facet_DR_Hist.png", path = "Dose Response",width = 10,height = 10,)
print(Facet_DR_Hist)


ggplot(data = binded_prevalences, mapping = aes(y= Prev, x=factor(Int,c('STOFF','OFFWash', 'OFFWr',  'STON', 'ONwash',  'ONWr','Exc') ), fill=factor(Int,c('STOFF','OFFWash', 'OFFWr',  'STON', 'ONwash',  'ONWr','Exc') )))+
  geom_violin()+
...
binded_prevalences_Tail<-binded_prevalences[binded_prevalences$Prev>.02,]

Tails_prev<-ggplot(data = binded_prevalences_Tail, mapping = aes(x= Prev, fill=factor(Int,c('STOFF','OFFWash', 'OFFWr',  'STON', 'ONwash',  'ONWr','Exc') )))+
...
ggsave(filename = "Tails_prev.png", path = "Dose Response",width = 10,height = 6,)
print(Tails_prev)
...
```{r}
ggplot(data =ST_STClosed_Analysis_Con, aes(x = Type, fill=Location)  )+
  geom_bar()+
  geom_text(stat = "count",aes(label=..count..),position = position_stack(vjust = .5))+
  xlab("Number of Fruit Items")+
  ylab("Intervention")+
