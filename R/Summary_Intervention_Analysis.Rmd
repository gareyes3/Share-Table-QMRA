...
Change the pathfolder input below and select the folder in which you want the information for this run to be save. Please just make a new folder in the /R folder. 
...
### Use the file Scenario Analysis-Published.Rdata.
...
#Sanitizer Analysis Revision 1/11 Figure 5

### Load in Rdata Sanitizer Analysis and Washing-Published.Rdata
```{r}
library(ggpubr)
library(rstatix)
...
#Boxplot of illness prevalences and significance analysis Figure 6

```{r}
All_binded_bplot<-rbind(
df_ill_Week_ON_Box_Prev,df_ill_Week_OFF_Box_Prev,df_ill_Week_ONWash_Box_Prev,df_ill_Week_ONWr_Box_Prev,df_ill_Week_OFFWash_Box_Prev,df_ill_Week_OFFWr_Box_Prev,df_ill_Week_Exc_Box_Prev, df_ill_Week_STClosed_Box_Prev, df_ill_Week_STAside_Box_Prev, df_ill_Week_TouchST_Box_Prev, df_ill_Week_WStation_Box_Prev, df_ill_Week_WBucket_Box_Prev,df_ill_Week_Sanitizer_Box_Prev)


Treatment_Names<-c("ON", "OFF", "ONWash","ONWr", "OFFWash","OFFWr", "XExc", "XSTClosed", 
                   "STAside", 'Touch', "TouchST", "WStation", "WBucket", "Sanitizer")
Treatment_Names_A<-c("ON", "OFF", "ONWash","ONWr", "OFFWash","OFFWr", "XExc", "XSTClosed", 
                   "STAside", "TouchST", "WStation", "WBucket", "Sanitizer")

All_binded_bplot$Treatments<-rep(Treatment_Names_A, each  =100000)

g<-ggplot(data = All_binded_bplot, aes(x = factor(Treatments, level = c('OFF', 'ON', 'OFFWash',"ONWash","OFFWr","ONWr","STAside","TouchST","WBucket", "XExc","WStation","Sanitizer","XSTClosed")), y = Prev))+
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = c(0.01079137), ymax = c(0.01836394),
           alpha = .3,fill = "grey")+
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = c(0.01204819), ymax = c(0.01929346),
           alpha = .3,fill =  "grey7")+
  geom_boxplot()+
  ylab("Illness prevalence from Dose response")+
  xlab("Scenarios")
...
g+
  ...
  geom_hline(aes(yintercept=0.01079137,linetype="Baseline Traditional"), color = "black", size=1, alpha = 0.8)+
  geom_hline(aes(yintercept=0.01836394,linetype="Baseline Traditional"), color = "black", size=1, alpha = 0.8)+
  geom_hline(aes(yintercept=0.01204819,linetype="Baseline ST"), color = "black", size=1, alpha = 0.8)+
  geom_hline(aes(yintercept=0.01929346,linetype="Baseline ST"), color = "black", size=1, alpha = 0.8)+
    #geom_hline(yintercept=c(0.01204819,0.01929346),linetype=c('dotted','dotted'), color = "black", size=1, alpha = 0.8)+
  scale_linetype_manual(name ="IQR for Baseline Scenarios",values = c("longdash", "dotted"))+
  theme_bw()+
  scale_x_discrete(labels = c("ON" = "2) Baseline share table" ,
                              "OFF" = "1)Baseline traditional cafeteria", 
                              "ONWash" = "4)Wash fruit, with share table",
                              "ONWr" = "6) Wrap fruit, with share table", 
                              "OFFWash" = "3) Wash fruit, no share table",
                              "OFFWr" = "5) Wrap fruit, no share table ", 
                              "XExc" = "10) Ill exclusion 50%", 
                              "XSTClosed" = "13) Re-Service off", 
                              "STAside" = "7) One-Way ST", 
                              "TouchST" = "8) Touches off , ST only", 
                              "WStation" = "11) Hand washing Station", 
                              "WBucket" = "9) Wash bucket", 
                              "Sanitizer" = "12) Hand sanitizer Station"))+
  theme(axis.text.x = element_text(angle =60, hjust = 1))+
  ggtitle("Illness prevalence per scenario") +
  theme(plot.title = element_text(hjust = 0.5,size = 15),text = element_text(size = 15))+
  ggsave(filename = "Boxplot_Prevalence.png", path = "C:/Users/Gustavo Reyes/Box/Share Table Project/QMRA Files/QMRA Paper/MRA Submission/Figures-R1",width = 10,height = 10,)

  
quantile(df_ill_Week_OFF_Box_Prev$Prev)
quantile(df_ill_Week_ON_Box_Prev$Prev)

model <- aov(Prev ~ Treatments, data = All_binded_bplot)
summary(model)

library(DescTools)
All_binded_bplot$Treatments<-as.factor(All_binded_bplot$Treatments)
DunnettTest(x=All_binded_bplot$Prev, g=All_binded_bplot$Treatments)

```


#Creation of the summary Table Table 3 
```{r}
...
#Food Waste Analysis With Locations Figure 2
```{r}
...
FoodWaste_Analysis<-data.frame(
  ...
  "ConsumedST" ="",
  "Reserviced Consumed"="",
  "Reserviced Landfilled"="",
  stringsAsFactors = FALSE
)

DF_Con_Trial<-func_remove_repeats(Individual_Analysis_Fr_CopON)
sum(DF_Con_Trial$Reserviced>0 & DF_Con_Trial$Location=="Discarded")

FoodWaste_Analysis[1,2:9]<-Function_FoodWaste_Full_Alt(Individual_Analysis_Fr_CopOFF)
FoodWaste_Analysis[2,2:9]<-Function_FoodWaste_Full_Alt(Individual_Analysis_Fr_CopON)
FoodWaste_Analysis[3,2:9]<-Function_FoodWaste_Full_Alt(Individual_Analysis_Fr_STClosed)
FoodWaste_Analysis[4,2:9]<-Function_FoodWaste_Full_Alt(Individual_Analysis_Fr_CopExc)
FoodWaste_Analysis[5,2:9]<-Function_FoodWaste_Full_Alt(Individual_Analysis_Fr_CopSTAside)
...
FoodWaste_Analysis_melted_No_RS<-FoodWaste_Analysis_melted%>%
filter(variable != "TotalReserviced") %>% 
group_by(Treatment) %>% 
mutate(pct=prop.table(value))

FoodWaste_Analysis_melted_PCT<-FoodWaste_Analysis_melted_No_RS%>% 
group_by(Treatment) %>% 
mutate(pct=prop.table(value)) 

FoodWaste_Analysis_melted_PCT$pct<-  round(FoodWaste_Analysis_melted_PCT$pct,3)
...
labelsFW <-c("TotalReserviced" = "Items Re-serviced", 
            ...
            "Reserviced.Consumed" = "Reserviced and Consumed",
            "ReservicedLandfilled" = "Reserviced and Landfilled"
               )

#Initial without percentages
ggplot(data = FoodWaste_Analysis_melted, aes(x= Treatment,y=value, fill= factor(variable, levels  = c("Reserviced.Consumed", "Reserviced.Landfilled","TotalReserviced", "TotalDiscarded", "ConsumedSel", "ConsumedST"))))+
  geom_col()+
  scale_x_discrete(labels=c("XClosed"="Re-Service Off",
                          "OFF" = "Traditional",
                          "ON"= "Share Table",
...
ggplot(data = FoodWaste_Analysis_melted_PCT, aes(x= Treatment,y=value, fill= factor(variable, levels  = c("Reserviced.Consumed", "Reserviced.Landfilled","TotalReserviced", "TotalDiscarded", "ConsumedSel", "ConsumedST"))))+
  ...
  ggsave(filename = "FoodSecFin4.png", path = "C:/Users/Gustavo Reyes/Box/Share Table Project/QMRA Files/QMRA Paper/MRA Submission/Figures-R1",width = 10,height = 8,)
