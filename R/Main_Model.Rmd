...
Individual_Analysis_Fr_Main<-Individual_Analysis_Fr_Main %>% 
    group_by(ID) %>% 
    filter(TotServices==max(TotServices))

Individual_Analysis_Fr_Main %>% 
  group_by(Location) %>% 
  count(Location)
...
Individual_Analysis_Fr_Main%>% 
    group_by(ConsumedAt) %>% 
    summarise(mean= mean(Contamination))

```

```{r}
Individual_Analysis_Fr_Main_C<-Individual_Analysis_Fr_Main %>% 
    group_by(ID) %>% 
    filter(TotServices==max(TotServices))%>% 
    filter(Location=="Consumed")


aaa<-Individual_Analysis_Fr_Main_C %>%
    dplyr::filter(grepl('WashedB', History))
```


```{r}
Individual_Analysis_Fr_CopWBucket
Individual_Analysis_Fr_CopOFF
Individual_Analysis_Fr_CopOFF

Individual_Analysis_Fr_CopWBucket_C<-Individual_Analysis_Fr_CopWBucket %>% 
    group_by(ID) %>% 
    filter(TotServices==max(TotServices))%>% 
    filter(Location=="Consumed")


Individual_Analysis_Fr_CopOFF_C<-Individual_Analysis_Fr_CopOFF %>% 
    group_by(ID) %>% 
    filter(TotServices==max(TotServices))%>% 
    filter(Location=="Consumed")


Individual_Analysis_Fr_CopON_C<-Individual_Analysis_Fr_CopON %>% 
    group_by(ID) %>% 
    filter(TotServices==max(TotServices))%>% 
    filter(Location=="Consumed")


Individual_Analysis_Fr_CopWBucket_C%>% 
    group_by(ConsumedAt) %>% 
    summarise(mean= mean(Contamination))
...
1096649572	+111675829	
1208325401

Individual_Analysis_Fr_CopOFF_C%>% 
    group_by(ConsumedAt) %>% 
    summarise(mean= mean(Contamination))
...
Individual_Analysis_Fr_CopON_C%>% 
    group_by(ConsumedAt) %>% 
    summarise(mean= mean(Contamination))
...
1000548939+342774396
1343323335


1208325401/1038608134
1343323335/1038608134


View(Individual_Analysis_Fr_CopWBucket_C)
